        ...
        app.init_resource::<ComputeEnabled>()
            ...
            .add_systems(
                ...
                (
                    ...
                    .chain() // Force l'ordre séquentiel
...
impl ComputeWorker for ParticleComputeWorker {
    fn build(world: &mut World) -> AppComputeWorker<Self> {
        ...
        let world_size = grid_params.width.max(grid_params.height).max(grid_params.depth);
        ...
        info!("Initializing compute worker with {} particles, {} types", num_particles, num_types);
        ...
        AppComputeWorkerBuilder::new(world)
            ...
            .add_pass::<ParticleComputeShader>(
                ...
                &[
                    ...
                    "max_force_range", "boundary_mode", "positions",
                    "velocities", "new_positions", "new_velocities",
                    "force_matrix", "food_positions", "food_count", "food_forces"
                ]
            )
            .build()
    }
...
fn update_compute_buffers(
    ...
    info!("GPU Update: {} particules, forces={}, nourriture={}",
          positions.len(),
          simulations.iter().next().map_or(0, |(_, g)| g.force_matrix.len()),
          food_positions.len());
}
...
fn apply_compute_results(
    ...
    for (i, (entity, mut transform, mut velocity)) in particles.iter_mut().enumerate() {
